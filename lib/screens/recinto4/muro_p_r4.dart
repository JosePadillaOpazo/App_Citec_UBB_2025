...
class Muro_Principal_R4 extends StatefulWidget {
  const Muro_Principal_R4({super.key});

  @override
  State<Muro_Principal_R4> createState() => _Muro_Principal_R4();
}

class _Muro_Principal_R4 extends State<Muro_Principal_R4> {
  GlobalKey canvaskeyImg2_Murop_R4 = GlobalKey();

  List<Offset?> _pointsImg2_PR4 = [];


  @override
  Widget build(BuildContext context) {
    final appState = context.watch<AppState>();
    final hojaActual = appState.obtenerHojaMuroPrincipal("Muro Eje Principal - Recinto 4");
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
                ...
                IconButton(
                  onPressed:() async {
                    final nuevoNombre = await appState.EditarNombre(
                      context,
                      appState.r4_murop_nombreController.text,
                    );

                    if (nuevoNombre != null && nuevoNombre.isNotEmpty) {
                      appState.actualizarNombreMuro(25,nuevoNombre);
                    }
                  },
          ...
          TextFormField(
            controller: appState.r4_murop_nombreController,
            enabled: false,
          ...
          Text(
            "🎯  Ubicación de Patologia Detectada",
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
          ),
                        ...
                        onPanUpdate: (details) {
                          setState(() {
                            ...
                            if (localPosition.dx >= 0 &&
                              ...
                              _pointsImg2_PR4 = List.from(_pointsImg2_PR4)..add(localPosition);
                            }
                          });
                        },
                        onPanEnd: (_) => setState(() => _pointsImg2_PR4.add(null)),
                        child: CustomPaint(
                          painter: DibujoPainter(_pointsImg2_PR4),
                        ),
                      );
                    },
                  ),
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () async {
                  await appState.guardarDibujoHojaPrincipal(
                    canvasKey: canvaskeyImg2_Murop_R4,
                    hoja: hojaActual,
                    imgnum: 2,
                    onGuardado: (file) {
                      setState(() {
